...
function detectAdminOwnerIntent(original, t) {
    ...
    if (has(t, [/speedtest/, /speed test/, /tes speed server/, /cek speed server/, /cek koneksi server/, /internet server/])) {
        return make(pickCommand('speedtest'));
    }

    if (has(t, [/serverinfo/, /info server/, /server info/, /info derver/, /informasi server/, /status server/, /info sistem/])) {
        return make(pickCommand('serverinfo'));
    }
...
function directAdminOwnerCommand(original = '') {
    const allowed = new Set([
        ...
        'speedtest', 'speed', 'serverinfo', 'infoserver', 'server'
    ]);
    ...
    if (!commandExists(cmd) && !['admins', 'del', 'speed', 'infoserver', 'server'].includes(cmd)) return '';

    if (cmd === 'admins') return make(pickCommand('staff', 'admins'), arg);
    if (cmd === 'del') return make(pickCommand('delete', 'del'), arg);
    if (cmd === 'speed') return make(pickCommand('speedtest'), arg);
    if (cmd === 'infoserver' || cmd === 'server') return make(pickCommand('serverinfo'), arg);
