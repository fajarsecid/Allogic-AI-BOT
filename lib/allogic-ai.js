...
const SYSTEM_PROMPT = `
IDENTITAS DAN BATASAN ALLOGIC AI BOT:
- Kamu adalah Allogic AI BOT, asisten WhatsApp berbasis multi-provider AI.
- Kamu boleh menjelaskan bahwa kamu memakai AI Router/Fallback jika ditanya.
- Jangan mengaku bisa belajar sendiri secara permanen.
- Jangan mengaku bisa mengakses internet, website, database, atau sumber real-time kecuali fitur/tool itu memang tersedia pada command bot.
- Jangan mengaku lebih pintar dari semua AI lain.
- Jelaskan kelebihan secara realistis: multi-provider, fallback model, command router, integrasi WhatsApp, dan bantuan coding/terjemahan/tanya jawab.
- Kalau ditanya kemampuanmu, jawab jujur dan jangan overclaim.
...
// ALLOGIC_GERMAN_TRANSLATION_FIX
function stripAiPrefixForTranslation(text = '') {
    return String(text || '')
        .replace(/^\s*(\.ai|\.ask|\.gpt|\.gemini|\.groq|\.allogic)\s*/i, '')
        .trim();
}

function detectGermanTranslationIntent(message = '') {
    let t = stripAiPrefixForTranslation(message)
        .replace(/[?!。.!]+$/g, '')
        .trim();
...
function buildGermanTranslationPrompt(message = '') {
    const term = detectGermanTranslationIntent(message);

    return `
TUGAS KHUSUS: TERJEMAHAN KE BAHASA JERMAN.

User meminta arti/terjemahan ke Bahasa Jerman.
...
- Jangan ubah kata menjadi possessive seperti "tripod's".
- Jika sumber adalah Bahasa Indonesia, terjemahkan ke Bahasa Jerman.
- Jika sumber adalah Bahasa Inggris, terjemahkan ke Bahasa Jerman.
- Berikan jawaban yang benar secara natural dalam Bahasa Jerman.
- Jika kata benda, sertakan artikel der/die/das dan plural jika umum.
- Jika kata kerja, berikan bentuk infinitiv.
- Jika ada beberapa padanan, berikan pilihan paling umum dulu.
- Jawaban harus pendek, jelas, dan cocok untuk WhatsApp.
...
Artinya:
<terjemahan contoh ke Indonesia>
`.trim();
}

function buildSmartUserMessage(message = '') {
    if (detectGermanTranslationIntent(message)) {
        return buildGermanTranslationPrompt(message);
    }
...
function formatForWhatsApp(text) {
    ...
    // Simpan code block dulu agar tidak rusak oleh formatter
    text = text.replace(/```([\s\S]*?)```/g, function(full, code) {
        const key = `@@CODEBLOCK${codeBlocks.length}@@`;
        codeBlocks.push(String(code || '').trim());
        return key;
    });

    // Bersihkan tag FINAL kalau ada
    text = text
        .replace(/<\/?FINAL>/gi, '')
        .replace(/\r\n/g, '\n')
        .trim();

    // Rapikan newline biasa
    text = text.replace(/\n{4,}/g, '\n\n\n');

    // Restore code block.
    // Support beberapa bentuk placeholder kalau sebelumnya sempat dirusak formatter lama.
    text = text.replace(/@@CODEBLOCK(\d+)@@/g, function(_, n) {
        const code = codeBlocks[Number(n)];
        if (code === undefined) return '';
        return '```html\n' + code + '\n```';
    });

    text = text.replace(/@@CODE_BLOCK_(\d+)@@/g, function(_, n) {
        const code = codeBlocks[Number(n)];
        if (code === undefined) return '';
        return '```html\n' + code + '\n```';
    });

    text = text.replace(/@@CODE_BLOCK(\d+)@@/g, function(_, n) {
        const code = codeBlocks[Number(n)];
        if (code === undefined) return '';
        return '```html\n' + code + '\n```';
    });
...
async function askGoogle(message, options = {}) {
    const cfg = getConfig();
    const local = (options.noLocal || detectGermanTranslationIntent(message)) ? null : simpleLocalReply(message);
    if (local) return local;
    ...
    contents.push({
        role: 'user',
        parts: [{ text: buildSmartUserMessage(message) }]
    });
...
async function askGroq(message, options = {}) {
    ...
    const messages = [
        ...
        ...history,
        { role: 'user', content: buildSmartUserMessage(message) }
    ];
...
async function askAllogicAI(message, options = {}) {
    // ALLOGIC_GERMAN_TRANSLATION_BYPASS_LOCAL
    if (detectGermanTranslationIntent(message)) {
        options = { ...options, noLocal: true };
    }
    const local = (options.noLocal || detectGermanTranslationIntent(message)) ? null : simpleLocalReply(message);
    if (local) return local;
...
// ALLOGIC_FORCE_V3_EXPORT_FINAL
try {
    if (typeof askAllogicAI_ModelFallbackV3 === 'function') {
        module.exports.askAllogicAI = askAllogicAI_ModelFallbackV3;
        console.log('[ALLOGIC] askAllogicAI forced to ModelFallbackV3');
    }
} catch (e) {
    console.log('[ALLOGIC_FORCE_V3_EXPORT_ERROR]', e.message);
}
