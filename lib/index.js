...
// ALLOGIC_ENV_OWNER_HELPER
function allogicLoadEnvOwners() {
    const fs = require('fs');
...
function allogicSenderNumber(jid = '') {
...
// ALLOGIC_FORCE_OWNER_EXPORT
try {
    const oldIsSudo = module.exports.isSudo;
    module.exports.isSudo = async function(sender) {
        const senderNum = allogicSenderNumber(sender);
        const owners = allogicLoadEnvOwners();

        if (owners.includes(senderNum)) return true;
        if (typeof oldIsSudo === 'function') {
            try {
                return await oldIsSudo(sender);
            } catch {
                return false;
            }
        }
...
// ALLOGIC_VPS_OWNER_FIX
try {
    const fs = require('fs');
    const path = require('path');

    function allogicReadEnvOwners() {
        const nums = new Set();
    ...
    module.exports.isSudo = async function(sender) {
        const senderNum = allogicNum(sender);
        const owners = allogicReadEnvOwners();

        console.log('[SUDO CHECK]', {
            sender,
            senderNum,
            owners,
            result: owners.includes(senderNum)
        });
    ...
    console.log('[ALLOGIC_VPS_OWNER_FIX ERROR]', e.message);
}

// ALLOGIC_AUTO_LID_OWNER_CHECK
try {
    const { isOwnerByNumberOrCachedLid, readEnvOwners, onlyNum } = require('./allogic-owner-resolver');
    const oldIsSudoAutoLid = module.exports.isSudo;

    module.exports.isSudo = async function(sender) {
        const senderNum = onlyNum(sender);
        const autoResult = isOwnerByNumberOrCachedLid(sender);

        console.log('[SUDO CHECK AUTO LID]', {
            sender,
            senderNum,
            owners: readEnvOwners(),
            cachedLid: global.allogicOwnerLidCache || {},
            result: autoResult
        });

        if (autoResult) return true;

        if (typeof oldIsSudoAutoLid === 'function') {
            try {
                return await oldIsSudoAutoLid(sender);
            } catch {}
        }
    ...
    console.log('[ALLOGIC_AUTO_LID_OWNER_CHECK ERROR]', e.message);
}
