...
function prepareAiQuery(query = '', options = {}) {
    ...
    const plain = stripAiPrefix(query).toLowerCase();

    const heavyQuestion =
        /(step by step|langkah demi langkah|matematika|persamaan|hitung|rumus|debug|error|bug|buatkan kode|buatkan game|script lengkap|analisis|jelaskan detail|secara detail|mendalam|arsitektur|database)/i.test(plain);

    if (isQuestionLike(query) && !isLikelyBotAction(query) && !heavyQuestion) {
        return {
            query,
            options: { ...options, forceLight: true },
            reason: 'normal_light_question'
        };
    }
